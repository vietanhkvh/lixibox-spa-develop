...
const component = (params = {}) => {
  const props = {
    cartStore: Object.assign({}, INITIAL_STATE_CART, {
      cartGiftList: cartList,
      specialAddOns: cartList,
  ...
  return withRouter((routerProps) => (
    <DiscountCodeProductRedeemModal {...Object.assign({}, props, routerProps, params)} />
  ));
};

describe('DiscountCodeProductRedeemModal', () => {
  describe(`when DiscountCodeGiftModal is open`, () => {
    test(`renders`, () => {
      expect(() => {
        reduxRender(component({ isDiscountCodeGiftModalOpen: true }), { initialState: {} });
      }).not.toThrow();
    });
  });

  describe(`when DiscountCodeAddonModal is open`, () => {
    test(`renders`, () => {
      expect(() => {
        reduxRender(component({ isDiscountCodeAddonModalOpen: true }), { initialState: {} });
      }).not.toThrow();
    });
  });
});
