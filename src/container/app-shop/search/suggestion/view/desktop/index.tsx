...
const View = ({
  ...
  // Extend panel width when displayed
  ...
  const adjustWidth = ({ panelBodyRef, isPanelVisible }) => {
    const panelBody = panelBodyRef.current;
    if (panelBody && isPanelVisible) {
      const panelBodyRect = panelBody.getBoundingClientRect();

      const delta = window.innerWidth - (panelBodyRect.x * 2 + panelBodyRect.width);
      panelBody.style.width = `${panelBodyRect.width + delta}px`;
    }
  };
  useEffect(() => {
    const panelBody = panelBodyRef.current;
    if (panelBody && !wasPanelVisible && isPanelVisible) {
      adjustWidth({ panelBodyRef, isPanelVisible });
    }
    if (wasPanelVisible && !isPanelVisible) {
      panelBody.style.width = '100%';
    }
  }, [panelBodyRef.current, isPanelVisible, wasPanelVisible]);
  useEffect(() => {
    const handleResize = () => {
      if (isPanelVisibleRef.current) {
        adjustWidth({ panelBodyRef, isPanelVisible: isPanelVisibleRef.current });
      }
    };
              ...
              {!keyword && !!trendingKeywords.length && (
                <KeywordRows
                  {...{
                    ...
                    classes: {
                      container: classNames(styles.section, styles.fullWidthSection),
                      title: styles.fullWidthSectionTitle,
                      rows: styles.fullWidthSectionRows,
                      item: styles.fullWidthSectionRow
                    },
