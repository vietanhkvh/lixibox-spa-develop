...
import { CustomCurrencyType, currenyFormat, formatCurrency } from 'utils/currency';
...
export const renderPurchaseType = (data) => {
...
export const renderCancelledTag = (title: string) => {
  ...
  return (
    <div className={styles.topInfo}>
      ...
      {isCustomerConfirmable && (
        <div className={styles.deliveryConfirmationBlock}>
          <SubmitButton
            {...{
              title: 'Đã nhận hàng',
              onSubmit: () => {
          ...
          />
          <div className={styles.hint}>Nếu đơn hàng này đã được giao thành công đến bạn</div>
        </div>
      )}
    ...
    <NavLink {...containerProps}>
      ...
      >
        ...
        <div style={STYLE.productItem.outerInfo}>
          ...
          <div style={STYLE.productItem.lowerSection}>
            <div style={STYLE.productItem.price}>
              {!isGift && <span style={STYLE.productItem.quantity}>{`${itemData.quantity} x `}</span>}
              {!isGift ? price : renderPurchaseType({ purchase_type: itemData.purchase_type })}
            </div>
...
export const support = ({ data, handleCancelOrder, cancellable }) => {
        ...
        data.payments.map((p) => (
          <div key={p.id} className={styles.generalSmallRow}>
            <div className={styles.generalRowSpaceBetween}>
              <div className={styles.generalRowBoldText}>{p.payment_method_name}:</div>
    ...
    <>
      <IconLayout icon={'receiver'}>
        <Top
          ...
          confirmOrderReceivedAction={confirmOrderReceivedAction}
...
const view = ({
  ...
  const OrderDetail = isOnlineOrder ? <OnlineOrder {...orderProps} /> : <StoreOrder {...orderProps} />;
  return (
    <div style={STYLE.main.container}>
      <div className={classnames(styles.iconLayoutList, { [styles.desktop]: !isMobileVersion() })}>{OrderDetail}</div>
      <ProductList
        data={data}
      ...
      {support({ data, handleCancelOrder, cancellable: data.actions && data.actions.cancellable })}
    </div>
  );
};
export default view;
