...
const component = (params = {}) => {
  const props = {
    ...
    productCountOnPage: 10,
    addItemToCartAction: jest.fn(),
    removeFromWaitListAction: jest.fn(),
    fetchUserWaitListAction: jest.fn(),
    openModalAction: jest.fn(),
    openAlertAction: jest.fn()
  };

  return <WaitlistProductItem {...Object.assign({}, props, params)} />;
};

describe('WaitlistProductItem', () => {
  test(`renders`, () => {
    expect(() => {
      reduxRender(component(), { initialState: {} });
    }).not.toThrow();
  });
  test('remove an item from wait list', async () => {
    const removeFromWaitListAction = jest.fn();
    const { container } = reduxRender(component({ removeFromWaitListAction }));
    const removeFromWaitListBtn = container.getElementsByClassName('buttonRemove')?.[0];
    expect(removeFromWaitListBtn).toBeInTheDocument();
    const user = userEvent.setup();
    await user.click(removeFromWaitListBtn);
    expect(removeFromWaitListAction).toBeCalled();
  });
});
