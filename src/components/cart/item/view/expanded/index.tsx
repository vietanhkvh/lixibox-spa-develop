...
function renderImage({ data, removeConfirmation, wishlistConfirmation, isReadOnly, isPrivateMode, onImageLinkClick }) {
  ...
  if (!!isPrivateMode) {
    return (
      <div className={className}>
        <Image className={styles.image} src={data.box.primary_picture.medium_url} alt={''} />
...
const DesktopActions = ({ removable, wishlistAddable, onPurchaseLater, onRemoveFromCart }) => {
  return (
    <div className={classNames(styles.desktopActions)}>
      {wishlistAddable && (
        <SubmitButton
          title="Mua sau"
          ...
          svgIcon="heart-line"
          testId={{ name: 'add-to-wishlist' }}
          onSubmit={onPurchaseLater}
        />
      )}
      {removable && (
        <SubmitButton
...
export default function renderComponent() {
  ...
  const nonRemovable = !!isEmptyKeyObject(data, 'removable') || !data.removable || isReadOnly;
      ...
      <div className={styles.sectionRight}>
        ...
        <div className={classNames(styles.rowBottom)}>
          <div className={classNames(styles.actions)}>
            ...
            {(wishlistAddable || !nonRemovable) && (
              <DesktopActions
                removable={!nonRemovable}
                wishlistAddable={wishlistAddable}
                onPurchaseLater={() => {
                ...
                onRemoveFromCart={() => this.setState({ removeConfirmation: true })}
              />
            )}
          </div>
          <div className={styles.meta}>
            {data.note && (
              <div className={styles.rowErrorMessage}>
                <div className={styles.message}>{data.note}</div>
              </div>
            )}
