...
class ImageSlide extends Component<IProps, IState> {
   ...
   */
  initDataSlide(_newImageList: Array<any> = this.state.imageList, posImg = 0) {
    if (!isMobileVersion()) {
  ...
  handleMouseHover() {
    const { data = [], column = 1 } = this.props;

    const preLoadImageList = Array.isArray(data)
      ? data.filter((item, $index) => $index >= column).map((item) => item.original_url)
      : [];
