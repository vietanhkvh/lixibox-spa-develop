...
const renderView = ({ props, state, handleMouseHover, selectSlide, navLeftSlide, navRightSlide }) => {
  const { magazineList } = state as IState;
  const { type, title, showViewMore, column, showHeader, isCustomTitle, titleStyle, onItemClick } = props as IProps;

  const renderMobileProps = { type, magazineList, column, onItemClick };
  ...
  const mainBlockProps = {
    ...
    showHeader,
    showViewMore,
    content: 0 === magazineList.length ? renderLoadingPlaceholder() : switchStyle[getDeviceVersion()](),
    style: {}
  };
